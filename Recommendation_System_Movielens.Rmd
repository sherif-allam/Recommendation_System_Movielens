---
title: \vspace{3.5in}Building Recommendation System using Machine Learning
...
```{r set-global-options, echo = FALSE,eval=TRUE}
...
```{r download-movielens,eval=TRUE, cache=TRUE, tidy=FALSE}
################################
...
```{r GENERAL_COMMENT,eval=TRUE, cache=TRUE, tidy=FALSE}
...
```{r movielens-structure, echo=TRUE,eval=TRUE, cache=TRUE,fig.width=20,fig.align='left',fig.show='asis',fig.height=6}
...
```{r movielens-head,eval=TRUE, echo=TRUE, message=TRUE, warning=TRUE}
...
```{r movielens-statistics-1, eval=TRUE, cache=TRUE}
...
```{r sample-7movies, eval=TRUE, cache=FALSE,results='asis'}
...
```{r sample-100movies, eval=TRUE, cache=FALSE}
...
```{r sample-100movies-to-matrix, eval=TRUE, cache=FALSE}
...
```{r movie-distribution,  eval=TRUE, cache=TRUE}
...
```{r user-distribution,  eval=TRUE, cache=TRUE}
...
```{r Uniqe_genres_list,  eval=TRUE, cache=TRUE}
...
```{r Uniqe_genres_list_Orderd,  eval=TRUE, cache=TRUE, tidy=FALSE}
...
```{r mutate-three_major_genres,  eval=TRUE, cache=TRUE,cache.lazy=FALSE}
...
```{r three_major_genres-distribution,  eval=TRUE, cache=TRUE, tidy=FALSE}
...
```{r mutate-year,  eval=TRUE, cache=TRUE,cache.lazy=FALSE}
...
```{r year-distribution,  eval=TRUE, cache=TRUE}
...
```{r save-movie-cleaned,  eval=TRUE, cache=TRUE}
...
```{r main-training-validation,  eval=TRUE, cache=TRUE}
...
```{r create-folds,  eval=TRUE, cache=TRUE}
# #
...
```{r balance-folds,  eval=TRUE, cache=TRUE}
# #
...
```{r RMSE-CUSTOM,  eval=TRUE, cache=TRUE}
...
```{r naive-rmses,  eval=TRUE, cache=TRUE}
...
```{r naive-rmses_result,  eval=TRUE, cache=TRUE, collapse=TRUE}
...
```{r rmse_pesults_1,  eval=TRUE, cache=FALSE}
...
```{r bi_check,  eval=TRUE, cache=TRUE}
...
```{r bi_calc,  eval=TRUE, cache=TRUE}
fold_list <- seq(1,9,1)
...
```{r bi_apply,  eval=TRUE, cache=TRUE ,collapse=TRUE}
...
```{r bi_sample,  eval=TRUE, cache=TRUE }
...
```{r bi_result,  eval=TRUE, cache=TRUE}
...
```{r bu_check,  eval=TRUE, cache=TRUE}
...
```{r bu_calc,  eval=TRUE, cache=TRUE}
fold_list <- seq(1,9,1)
...
```{r bu_result,  eval=TRUE, cache=TRUE}
# # Get the Training set that achieve the best performance
training_set <- edx[Training_Index_List[[fold_n]],]
...
```{r bv_check,  eval=TRUE, cache=TRUE}
...
```{r bv_calc,  eval=TRUE, cache=TRUE}
...
Next, is to calculate the prediction on the test dataset as the following:
```{r bv_result,  eval=TRUE, cache=TRUE}
# # Get the Training set that achieve the best performance
training_set <- edx[Training_Index_List[[fold_n]],]
...
```{r bq_check,  eval=TRUE, cache=TRUE}
...
```{r bq_calc,  eval=TRUE, cache=TRUE}
...
Next, is to calculate the prediction on the test dataset as the following:
```{r bq_result,  eval=TRUE, cache=TRUE}
# # Get the Training set that achieve the best performance
training_set <- edx[Training_Index_List[[fold_n]],]
...
```{r bx_result,  eval=TRUE, cache=TRUE}
...
```{r bx-to-df,  eval=TRUE, cache=TRUE}
...
```{r bx-plot,  eval=TRUE, cache=TRUE}
...
```{r bx-test_result, eval=TRUE, cache=TRUE, tidy=FALSE}
rmse_pesults_tb <- 
...
```{r MFactor, eval=TRUE, cache=TRUE}
...
```{r MFactor-TRIAN, eval=TRUE, cache=TRUE}
if(!file.exists("mtx_fact_pred_ratings.rda")){
r = Reco()
# set tune parameters
...
```{r MFactor-Pred, eval=TRUE, cache=TRUE}
...
```{r MFactor-RMSE, eval=TRUE, cache=TRUE}
...
```{r SVD-Prep1, eval=TRUE, cache=TRUE}
...
```{r SVD-Prep2, eval=TRUE, cache=TRUE, results="hide",tidy=FALSE}
if(!file.exists("decomposed_matrix.rda")){
...
```{r SVD-Prep3, eval=TRUE, cache=TRUE, ,results="hide"}
if(!file.exists("decomposed_matrix.rda")){
...
```{r SVD-Prep4, eval=TRUE, cache=TRUE, tidy=FALSE}
if(!file.exists("decomposed_matrix.rda")){
...
```{r SVD-Prep5, eval=TRUE, cache=TRUE}
if(!file.exists("decomposed_matrix.rda")){
...
```{r SVD-Prep6, eval=TRUE, cache=TRUE, result=FALSE}
if(!file.exists("decomposed_matrix.rda")){
...
```{r SVD-DECOMPOSE, eval=TRUE, cache=TRUE}
if(!file.exists("decomposed_matrix.rda")){
...
```{r SVD-VAR_CHECK_PLOT, eval=TRUE, cache=TRUE}
...
```{r SVD-TRAIN, eval=TRUE, cache=TRUE}
...
```{r SVD-PLOT_TRAIN_VS_VALD, eval=TRUE, cache=TRUE,, tidy=FALSE}
...
```{r SVD-RMSE, eval=TRUE, cache=TRUE}
...
```{r SVD-RMSE_COMP, eval=TRUE, cache=TRUE}
